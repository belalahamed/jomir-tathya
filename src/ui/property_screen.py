"""
Property Screen Module is UI module to load the Propery Lookup UI
"""
...
def _run_webview_viewer(html_content, should_print=False):
    ...
    window = webview.create_window('Property Details', html=html_content, width=1000, height=800)
...
class PropertyScreen(ctk.CTkToplevel):
    """Property Screen UI"""
    ...
    def __init__(self, session_cookies=None):
        ...
        self.title("Know Your Property - Jomir Tathya")
        self.geometry("800x700")
        self.configure(fg_color="#F5F5F7")  # Configure Property Screen background color
        ...
        # Results Panel
        self.results_frame = ctk.CTkFrame(self, fg_color="transparent", corner_radius=10)
        self.results_frame.pack(side="right", expand=True, fill="both", padx=20, pady=20)
        
        self.results_label = ctk.CTkLabel(self.results_frame, text="Search Results", font=("Calibri", 18, "bold"), text_color="black")
    ...
    def __fetch_and_update_districts(self):
        ...
        try:
            ...
            self.after(0,lambda: self.status_label.configure(text="Failed to load districts!", text_color="red"))
    ...
    def _update_district_options_ui(self, districts_list):
        ...
        if self.districts_list:
            ...
            district_names = [district.get("eng_dname")for district in self.districts_list if district.get("eng_dname")]
            ...
            self.status_label.configure(text="Districts loaded successfully!", text_color="green")
    ...
    def _on_district_selected(self, selected_district_name):
        ...
        if selected_district_name == "Select District":
            ...
            selected_district = next((district for district in self.districts_list if district.get("eng_dname") == selected_district_name), None)
    ...
    def __fetch_and_update_blocks(self):
        ...
        try:
            ...
            self.after(0, lambda: self.status_label.configure(text="Failed to load blocks!", text_color="red"))
    ...
    def _update_block_options_ui(self, blocks_list):
        ...
        if self.blocks_list:
            block_names = [b.get("eng_bname") for b in self.blocks_list if b.get("eng_bname")]
            block_options = ["Select Block"] + block_names
            self.block_option_menu.configure(values=block_options, state="normal")
            self.block_option_menu.set("Select Block")
            self.status_label.configure(text="Blocks loaded successfully.", text_color="green")
    ...
    def _on_block_selected(self, selected_block_name):
        ...
        if selected_block_name == "Select Block":
            ...
            selected_block = next((block for block in self.blocks_list if block.get("eng_bname") == selected_block_name), None)
    ...
    def __fetch_and_update_mouzas(self):
        ...
        try:
            ...
            self.after(0, lambda: self.status_label.configure(text="Failed to load mouzas!", text_color="red"))
    ...
    def _update_mouza_options_ui(self, mouzas_list):
        ...
        if self.mouzas_list:
            mouza_names = [mouza.get("mouName") for mouza in self.mouzas_list if mouza.get("mouName")]
            mouza_options = ["Select Mouza"] + mouza_names
            self.mouza_option_menu.configure(values=mouza_options, state="normal")
            self.mouza_option_menu.set("Select Mouza")
            self.status_label.configure(text="Mouzas loaded successfully.", text_color="green")
            
        else:
            self.mouza_option_menu.configure(values=["No Mouzas Found"], state="disabled")
    ...
    def _on_mouza_selected(self, selected_mouza_name):
        ...
        if selected_mouza_name == "Select Mouza":
            ...
            selected_mouza = next((mouza for mouza in self.mouzas_list if mouza.get("mouName") == selected_mouza_name), None)
    ...
    def _launch_webview(self, should_print=False):
        ...
        try:
            status_text = "Opening PDF viewer..." if should_print else "Opening viewer..."
            self.status_label.configure(text=status_text, text_color="blue")
            html_content = self.current_html
            
            if not html_content or len(html_content.strip()) < 100:
                err_text = "No results found to save!" if should_print else "No results found to open!"
            ...
            p = multiprocessing.Process(target=_run_webview_viewer, args=(html_content, should_print), daemon=True)
            p.start()
            
            success_text = "Print dialog opened." if should_print else "Opened in pywebview viewer."
