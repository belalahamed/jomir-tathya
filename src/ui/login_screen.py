...
class LoginScreen(ctk.CTkToplevel):
    ...
    def __init__(self):
        ...
        self.title("Login - Jomir Tathya")
        self.geometry("450x700")
        self.configure(fg_color="#F5F5F7") # macOS background color
    ...
    def view_login_ui(self):
        ...
        self.header_logo = ctk.CTkLabel(
            self, text="Jomir Tathya", font=("Calibri", 40, "bold"), text_color="#007AFF"
        )
        self.header_logo.pack(pady=(30, 10))
        ...
        self.captcha_frame = ctk.CTkFrame(self.login_frame, fg_color="transparent", border_color="#D1D1D6")
        self.captcha_frame.pack(pady=10)

        self.captcha_label = ctk.CTkLabel(self.captcha_frame, text="")
        self.captcha_label.pack(side="left")
        self.captcha_label.bind("<Button-1>", self.refresh_captcha) # Click image to refresh
    ...
    def handle_send_otp(self, event=None):
        ...
        if username and password and captcha and self.send_otp_btn.cget("state") == "normal":
            self.send_otp_btn.configure(state="disabled", text="Sending...")
            self.status_message_label.configure(text="Requesting OTP...", text_color="#1D1D1F")
            
            # Run network call in a separate thread
            threading.Thread(target=self._send_otp_thread, args=(username, password, captcha), daemon=True).start()
    ...
    def _send_otp_thread(self, username, password, captcha):
        try:
            ...
            if send_otp_response.get("message") == "OK":  
                ...
                self.after(0, lambda: self._on_otp_fail("OTP generation failed! Check credentials."))
    ...
    def _on_otp_success(self):
        ...
        self.status_message_label.configure(text="OTP generated successfully!", text_color="green")
    ...
    def handle_login(self, event=None):
        ...
        if otp and self.login_btn.cget("state") == "normal":
            ...
            threading.Thread(target=self._login_thread, args=(username, password, otp), daemon=True).start()
    ...
    def _login_thread(self, username, password, otp):
        try:
            ...
            if validate_otp_response.get("checkmsg") == "success":
                self.after(0, lambda: self.status_message_label.configure(text="Login successful!", text_color="green"))
                # Trigger the transition callback after a short delay
                self.after(0, self._on_login_success)
            else:
                self.after(0, self._on_login_fail)
        except Exception as e:
            self.after(0, lambda: self.status_message_label.configure(text="Login Error: Connection lost.", text_color="red"))
    ...
    def _on_login_fail(self):
        ...
        self.status_message_label.configure(text="Login failed! Invalid OTP.", text_color="red")
